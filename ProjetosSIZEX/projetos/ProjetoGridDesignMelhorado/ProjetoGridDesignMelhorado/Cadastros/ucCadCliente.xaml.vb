...
#Region "Eventos Buttons - SUB"
    ...
    Private Sub PesTxt_TextChanged(sender As Object, e As TextChangedEventArgs) Handles PesTxt.TextChanged
        If objCliente.lstClientes.Count > 0 Then
            If PesLbl.Content = "Pesquisa (Nome):" Then
                srcCliente.Source = objCliente.lstClientes.Where(Function(p) p.Nome.Contains(PesTxt.Text)).ToList
            ElseIf PesLbl.Content = "Pesquisa (CPF):" Then
                srcCliente.Source = objCliente.lstClientes.Where(Function(p) p.CPF.Contains(PesTxt.Text)).ToList
            ElseIf PesLbl.Content = "Pesquisa (Endereço):" Then
                srcCliente.Source = objCliente.lstClientes.Where(Function(p) p.Endereco.Contains(PesTxt.Text)).ToList
            Else
